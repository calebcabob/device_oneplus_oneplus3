...
AQUQARIOS_ROOT="$MY_DIR"/../../..

HELPER="$AQUQARIOS_ROOT"/vendor/aquarios/build/tools/extract_utils.sh
if [ ! -f "$HELPER" ]; then
...
setup_vendor "$DEVICE" "$VENDOR" "$AQUQARIOS_ROOT"
...
CAMERA_HAL="$AQUQARIOS_ROOT"/vendor/"$VENDOR"/"$DEVICE"/proprietary/lib/hw/camera.msm8996.so
